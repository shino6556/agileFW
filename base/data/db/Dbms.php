...
abstract class Dbms {
	...
	public function create(string $table, array $params, array $types, bool $isMultiRows = false): int {
		...
		// SQL文の実行
		$result = $this->execute(__METHOD__, $sql, $paramList, $types);
	...
	public function delete(string $table, int|array $pkeys, string $pkeyName = 'pkey'): int {
		...
		// SQL文の実行
		$result = $this->execute(__METHOD__, $sql, $pkeys, [$pkeyName => Types::INT]);
		return $this->end(__METHOD__, $result);
	}
	 ...
	 */
	protected function execute(string $method, string $sql, array $paramList, array $types): DbErrors {
		$this->start($method, $sql);
		$result = 1;
		try {
			// SQLステートメント生成
			$statement = self::$PDO->prepare($sql);
			if (!$statement) {
				return $this->error($method, DbErrors::ERR_STATEMENT->name, $sql);
			}
			// 値のバインドと実行
			foreach ($paramList as $params) {
				$ret = $this->bindValue($statement, $params, $types);
				if (!$ret) {
					return $this->error($method, DbErrors::ERR_BIND->name);
				}
				$ret = $statement->execute();
				if (!$ret) {
					return $this->error($method, DbErrors::ERR_EXECUTE->name);
				}
			}
		} catch (PDOException $ex) {
			return $this->error($method, DbErrors::ERR_PDO->name, $ex->getMessage(), $ex);
		}
		return $this->end($method, $result);
	}

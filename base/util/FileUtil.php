...
class FileUtil {
	 ...
	 * @param callable $callback コールバック [インスタンス,'メソッド名'] method(int|string $idx, mixed $value) : mixed;
	 * @return array 処理結果
	 */
	public static function load(string $path, callable $callback): array {
	 ...
	 * @param callable $callback コールバック [インスタンス,'メソッド名'] method(int|string $key, mixed $value) : string;
	 * @param array $values 処理する配列
	 * @return bool true:処理成功
	 */
	public static function save(string $path, callable $callback, array $values): bool {
		self::self()->start(__METHOD__);
		$result = false;
		try {
			$fp = fopen($path, 'w');
	 ...
	 * @param array  $fileInfoList ファイル情報 ['name' => '元のファイル名', 'tmp_name' => '一時ファイル名']
	...
	public static function upload(array $fileInfo, string $uploadDir, string $prefix = ''): string {
		self::self()->start(__METHOD__, 'args=' . StrUtil::toString($fileInfo), $uploadDir, $prefix);

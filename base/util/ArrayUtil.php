...
class ArrayUtil {
	 ...
	 * @param mixed $array アクセスする配列であるはずのもの
	 * @param mixed $key 要素のキーであるはずのもの
	 * @return ?bool true:有効な場合 false:値がnullの場合 null:キーが存在しない売
	 */
	public static function enable(mixed $array, mixed $key): ?bool {
		...
		if (!in_array($key, array_keys($array))) return null;
	...
	/**
	 * 配列を連結する
	 * @param array $target 連結対象の配列
	 * @param array $keyVals [キー=>値, ...]の配列
	 * @return array 連結された配列
	 */
	public static function append(array $target, array $keyVals): array {

...
final class DateUtil {
	...
	/** @var string 標準の書式(年月日時分秒) */
	public const string YmdHis = 'Y-m-d H:i:s';

	/** @var string 標準の書式(年月日) */
	public const string Ymd = 'Y-m-d';

	/** @var string 標準の書式(時刻のみ) */
	public const string His = 'H:i:s';

	/** @var string 標準の書式(日時分) */
	public const string YmdHi = 'Y-m-d H:i';

	/** @var string 標準の書式(日時マイクロ秒) */
	public const string YmdHisU = 'Y-m-d H:i:s.u';

	/** @var string 標準の書式(タイムスタンプ) */
	public const string Timestamp = 'YmdHisu';
	 ...
	 * @return string
	 */
	public static function toString(DateTime $val, string $format = self::YmdHis): string {
	...
	/**
	 * 日時に変換可能な文字列かどうかを返す。
	 * @param string|DateTime $value チェックする文字列
	 * @return bool true:変換可能
	 */
	public static function isDateTime(string|DateTime $value): bool {
	...
	/**
	 * 今日の日付を得る。時分秒ミリ秒は０。
	 * @param ?string $duration 今日から前後にずらす('n days|months|years') 省略=ずらさない 
	 * @param bool $isAfter 後にずらす 省略=前にずらす
	 * @return DateTime
	 */
	public static function today(?string $duration = null, bool $isAfter = false): DateTime {
		$date = self::now();
		$date->setTime(0, 0, 0, 0);
		if ($duration) {
			$date = self::move($date, $duration,  $isAfter);
		}
		return $date;
	}
	/**
	 ...
	 * @return DateTime
	 */
	public static function move(DateTime $date, string $duration, bool $isAfter = false) {
	 ...
	 * @return string
	 */
	public static function toStrJP(DateTime $val, string $format = self::YmdHis, bool $isShort = false): string {
		// 年号を検索する
	...
	/**
	 * 左辺から右辺を引いた日時の間隔を取り、書式に従って文字列化、必要なら整数値で返す。
	 ...
	 * @param ?DateTime $now 現時点 省略=null : DateUtil::now()
	 * @return int 年齢 
	 */
	public static function age(DateTime $birthday, ?DateTime $now = null): int {
		if ($now === null) {
			$now = self::now();
		}
		$age = self::diff($birthday, $now, DateFmt::DIFF_YEAR, true);
		return $age;
	}
}
/**

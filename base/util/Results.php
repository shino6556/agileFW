...
class Results {
	 ...
	 */
	public static function error(string $name, string $message, mixed ...$params): ResultInfo {
		return self::trace(-1, $name,  $message,  ...$params);
	}
	 ...
	 * @return ResultInfo 処理結果情報
	 */
	public static function warning(string $name, string $message, mixed ...$params): ResultInfo {
		return self::trace(0, $name,  $message,  ...$params);
	}
	 ...
	 * @return ResultInfo 処理結果情報
	 */
	public static function trace(int $level, string $name, string $message, mixed ...$params): ResultInfo {
		$info = new ResultInfo();
		...
		foreach ($params as $idx => $param) {
			if ($param instanceof Throwable) {
				$msg = self::$self->getErrorInfo($param);
				$params[$idx] = $msg;
			}
		}
		$info->params = $params;
